...
export const CreateShopModal: React.FC<CreateShopModalProps> = ({
  ...
  const handleSingleSubmit = (keepOpenAfter: boolean = false) => {
    ...
    // Auto-generate license key with unlimited product capacity
    const newLic = generateLicense({
      ...
      notes: `Unlimited license provisioned for ${shopName.trim()}`,
    });
    ...
    setSuccessMsg(`Shop "${shopName}" provisioned with Unlimited Catalog capacity!`);
  ...
  const handleBatchSubmit = () => {
    ...
    setSuccessMsg(`Success! ${res.count} stores provisioned simultaneously with Unlimited capacity.`);
    setTimeout(() => {
      onClose();
    }, 1200);
  };
          ...
          >
            ...
            <div>
              <label className="block text-slate-300 font-semibold mb-2 uppercase text-[11px] tracking-wider">
                2. Select Business Archetype & Template:
              </label>
