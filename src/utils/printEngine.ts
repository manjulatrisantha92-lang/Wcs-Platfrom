  ...
  <div class="totals-box">
    <div class="totals-inner">
      ...
      ${loyaltyRedeemedVal > 0 ? `
        <div class="totals-row" style="color: #b45309; font-weight: bold;">
          <span>(-) Less Loyalty Points:</span>
          <span style="font-family: monospace;">-${currencySymbol} ${loyaltyRedeemedVal.toFixed(2)} (${loyaltyRedeemedVal} pts)</span>
        </div>
      ...
      ${(loyaltyRedeemedVal > 0 || loyaltyEarnedVal > 0 || customerLoyaltyBalanceVal !== undefined) ? `
        <div style="border-top: 1px dashed #cbd5e1; padding-top: 6px; margin-top: 6px; font-size: 10px;">
          ...
          <div style="display: flex; justify-content: space-between;"><span>Bill Total:</span><span>${currencySymbol} ${grandTotalVal.toFixed(2)}</span></div>
          ${loyaltyRedeemedVal > 0 ? `<div style="display: flex; justify-content: space-between; color: #b45309; font-weight: bold;"><span>(-) Loyalty Redeemed:</span><span>-${currencySymbol} ${loyaltyRedeemedVal.toFixed(2)} (${loyaltyRedeemedVal} pts)</span></div>` : ''}
          <div style="display: flex; justify-content: space-between; font-weight: 900; border-top: 1px dotted #cbd5e1; padding-top: 2px;"><span>(=) Balance (Total - Loyalty):</span><span>${currencySymbol} ${netAfterLoyaltyVal.toFixed(2)}</span></div>
          ${loyaltyEarnedVal > 0 ? `<div style="display: flex; justify-content: space-between; color: #047857; font-weight: bold; margin-top: 2px;"><span>(+) Points Earned This Bill:</span><span>+${loyaltyEarnedVal.toLocaleString()} pts</span></div>` : ''}
          ${customerLoyaltyBalanceVal !== undefined ? `<div style="display: flex; justify-content: space-between; font-weight: bold; border-top: 1px dotted #cbd5e1; margin-top: 2px; padding-top: 2px;"><span>Customer Loyalty Balance:</span><span>${customerLoyaltyBalanceVal.toLocaleString()} pts</span></div>` : ''}
        </div>
      ` : ''}
    </div>
  </div>
...
<body>
  ...
  ${loyaltyRedeemedVal > 0 ? `
    ...
    <div class="totals-row receipt-total" style="font-size: 12px; margin: 2px 0;">
      <span>(=) BALANCE (TOTAL - LOYALTY):</span>
  ...
  ${(loyaltyRedeemedVal > 0 || loyaltyEarnedVal > 0 || customerLoyaltyBalanceVal !== undefined) ? `
    ...
    <div style="font-size: 9px; margin: 2px 0;">
      ...
      ` : ''}
      <div class="totals-row bold" style="border-top: 1px dotted #666; padding-top: 1px;">
        <span>(=) Balance (Total - Loyalty):</span>
        <span>${netAfterLoyaltyVal.toFixed(2)}</span>
      </div>
      ${loyaltyEarnedVal > 0 ? `
      ...
      ${customerLoyaltyBalanceVal !== undefined ? `
        <div class="totals-row bold" style="border-top: 1px dotted #666; margin-top: 2px; padding-top: 1px;">
