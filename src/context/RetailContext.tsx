...
export const RetailProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const generateLicense = (params: {
    ...
    const year = new Date().getFullYear() + (params.durationMonths ? Math.round(params.durationMonths / 12) : 1);
    const generatedKey = params.customKey?.trim().toUpperCase() || `WCS-${prefix}-LK-${randomNum}-${year}`;
